...
class Alias{
	...
	function add_addresses($user_id,$email_address,$contact_name, $mailbox){
		...
		mysql_query($inserta_query);
			...
			if($mailbox == "new"){
			//create folder for the created email address
			_imp_createFolder(imap_utf7_encode($email_address));
			$mailbox = $emailwdom;
			}

		$list = IMP_Fetchmail::listDrivers();
	
		$fetchmail = new IMP_Fetchmail(array('KPZBezT5', 'imap', 'IMAP/POP3 Mail Servers', 'IMAP(Auto Detect Protocols)', 'KPZBezT5@example.com', 'MD5(john123)', 'localhost','Inbox', 'KPZBezT5', 'True', 'False', 'False', 'True', 'acctcolor'
	    ));
	...
	//add the address details (group adresses)
	...
	Alias::add_from_address($email, $mailbox);
        ...
        //add address details(domain addresses)
...
//add group filters
	function add_group_filter($userid,$emailwdom,$contact_email, $mailbox){
		...
		$srow = mysql_fetch_assoc($sres);
         ...
         //set the preferences to set the from address for displayed
